# Packages importing
#%%
import numpy as np
import pandas as pd
import pickle
import tkinter
import matplotlib
# matplotlib.use('TkAgg')  # !IMPORTANT
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
import rasterio
from rasterio.plot import show
import seaborn as sns
import scipy, os
from scipy import stats
from sklearn.preprocessing import StandardScaler, MinMaxScaler
from tabulate import tabulate
from sklearn.decomposition import PCA
from sklearn.linear_model import LinearRegression
from sklearn.cluster import KMeans
from sklearn.metrics import confusion_matrix, classification_report
from sklearn.linear_model import LogisticRegression
from sklearn.ensemble import RandomForestClassifier
from xgboost import XGBClassifier
from toolbox import get_train_val_ps, important_features, contextual_features, tif_to_df, cm
random_seed=123
target_names = ['Not Deprived', 'Deprived']

#Helper
def plot_pred_area(nrow,ncol,pred):
    pred_map = pred.reshape(nrow, ncol)
    values = np.unique(pred_map.ravel())
    im = plt.imshow(pred_map, interpolation='none')
    colors = [im.cmap(im.norm(value)) for value in values]
    patches = [mpatches.Patch(color=colors[i], label="Class {l}".format(l=values[i])) for i in range(len(values))]
    plt.legend(handles=patches, bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
    plt.show()

def confused_matrix(y_test,y_pred):
    cm=confusion_matrix(y_test, y_pred)
    ax= plt.subplot()
    sns.heatmap(cm, annot=True, fmt='g', ax=ax)  #annot=True to annotate cells, ftm='g' to disable scientific notation
    ax.set_xlabel('Predicted labels');ax.set_ylabel('True labels')
    ax.set_title('Confusion Matrix')
    ax.xaxis.set_ticklabels(['Not Deprived', 'Deprived'])
    ax.yaxis.set_ticklabels(['Not Deprived', 'Deprived'])
    plt.show()
#%%
# Load Data into Pandas Dataframe
lagos= pd.read_parquet('lagos_df.parquet.gzip')
lagos= lagos[~(lagos==-9999.000000)]

# Check for Nulls $ replace with mean
print(lagos.isna().sum().sort_values(ascending=False))
# lagos.fillna(0,inplace=True)
# Replace Nulls with mean values of respective columns
values= {i:lagos[i].mean() for i in lagos.columns[:-2]}
lagos.loc[:,lagos.columns[:-2]]= lagos.loc[:,lagos.columns[:-2]].fillna(value=values)
print(f'Null values: {lagos.isna().sum().any()}')

# Split Train, Validation & Test
train= lagos[lagos.type=='train'].copy(deep=True).drop('type',axis=1)
validation= lagos[lagos.type=='validation'].copy(deep=True).drop('type',axis=1)
test= lagos[lagos.type=='test'].copy(deep=True).drop('type',axis=1)

#%%
# Get important features for contextual features from PCA and VIF
from toolbox import  important_38,important_6, important_62, important_104
important_104= important_104()
important_38= important_38()
important_62= important_62()
important_6= important_6()
important_104.append('labels')
important_38.append('labels')
important_62.append('labels')
important_6.append('labels')

#%%
# Get the important 104 features
train_c= train.loc[:,important_62].copy()
validation_c= validation.loc[:,important_62].copy()
test_c= test.loc[:,important_62].copy()

#%%
# Logistic Regression | Random Forest Classifier | XGBoost Classifier
 # Get data in numpy array
# Split into features and target
x_train, y_train= train_c.iloc[:,:-1].values, train_c.iloc[:,-1].values
x_val, y_val= validation_c.iloc[:,:-1].values, validation_c.iloc[:,-1].values
x_test, y_test= test_c.iloc[:,:-1].values, test_c.iloc[:,-1].values
print(x_train.shape,x_val.shape,x_test.shape)

# x_train, y_train= train.iloc[:,:-1].values, train.iloc[:,-1].values
# x_val, y_val= validation.iloc[:,:-1].values, validation.iloc[:,-1].values
# x_test, y_test= test.iloc[:,:-1].values, test.iloc[:,-1].values
# print(x_train.shape,x_val.shape,x_test.shape)

#%%
# Scale features
mms =MinMaxScaler()
x_train = mms.fit_transform(x_train)
x_val = mms.transform(x_val)
x_test = mms.transform(x_test)
#Save models
with open('mms62.pickle','wb') as f:
    pickle.dump(mms, f)
# with open('mms104.pickle', 'rb') as f:
#     mms= pickle.load(f)
#%%
# Models (based on Random Search CV)
rfc= RandomForestClassifier(class_weight='balanced', max_depth=30,
                            min_samples_leaf=20, min_samples_split=100,random_state=123)
lr= LogisticRegression(C=6.9098485502686176, class_weight='balanced',
                       random_state=123, solver='sag', tol=10.792035546783445)
xgbc= XGBClassifier(n_estimators=30, max_depth=20,
                    eval_metric='logloss',random_state=random_seed)

rfc.fit(x_train,y_train)
y_pred_rfc = rfc.predict(x_test) # predict test set
#%%
lr.fit(x_train,y_train)
y_pred_lr = lr.predict(x_test) # predict test set
#%%
xgbc.fit(x_train,y_train)
y_pred_xg = xgbc.predict(x_test) # predict test set
#%%
# Ensembling 3 models
rez= pd.DataFrame()
rez['res1']=np.array(y_pred_lr)
rez['res2'] = np.array(y_pred_xg)
rez['res3'] = np.array(y_pred_rfc)
rez_final= rez.mode(axis=1).values
rez_final= rez_final.ravel()
#%%
confused_matrix(y_test, rez_final) # Confusion matrix
print(classification_report(y_test, rez_final, target_names=target_names))

#%%
# FOR 1 MODEL AT A TIME
area0= [f"/home/ubuntu/capstone/Lagos/lagos_contextual_100m/spfea_100m/lag_area11.tif"]
area0_mask= [f"/home/ubuntu/capstone/Lagos/lagos_contextual_100m/mask_100m/lag_area11.tif"]
cf= contextual_features()
area0_df= tif_to_df(area0,area0_mask,cf)
area0_pred= lr.predict(mms.transform(area0_df.drop(['labels','pantex_sc3_min'],axis=1).values))

#plot original area
img= rasterio.open(area0_mask[0]).read(1)
show(img)
#Plot predicted area
plot_pred_area(95,95,area0_pred)
#Plot confusion matrix
confused_matrix(area0_df.labels.values, area0_pred)

#%%
# ENSEMBLE MODEL
area0= ["/home/ubuntu/capstone/nairobi/nairobi/nairobi_processed_TrainData_10m/spfea_144/nai_area28.tif"]
area0_mask= ["/home/ubuntu/capstone/nairobi/nairobi/nairobi_processed_TrainData_10m/mask/nai_area28.tif"]
cf= contextual_features()
area0_df= tif_to_df(area0,area0_mask,cf)
area0_pred1= lr.predict(mms.transform(area0_df.drop(['labels','pantex_sc3_min'],axis=1).values))
area0_pred2= rfc.predict(mms.transform(area0_df.drop(['labels','pantex_sc3_min'],axis=1).values))
area0_pred3= xgbc.predict(mms.transform(area0_df.drop(['labels','pantex_sc3_min'],axis=1).values))
reze= pd.DataFrame()
reze['res1']=np.array(area0_pred1)
reze['res2'] = np.array(area0_pred2)
reze['res3'] = np.array(area0_pred3)
rez_finale= reze.mode(axis=1).values
rez_finale= rez_finale.ravel()
# area0_pred= lr.predict(mms.transform(area0_df.drop(['labels','pantex_sc3_min'],axis=1).values))

#plot original area
img= rasterio.open(area0_mask[0]).read(1)
show(img)
#Plot predicted area
plot_pred_area(95,95,rez_finale)
#Plot confusion matrix
confused_matrix(area0_df.labels.values, rez_finale)

#%%
#Save models
with open('model_lr_144.pickle','wb') as f:
    pickle.dump(lr, f)
with open('model_rfc_144.pickle','wb') as f:
    pickle.dump(rfc, f)
with open('model_xgbc_144.pickle','wb') as f:
    pickle.dump(xgbc, f)

# loading model
# with open('model_lr_97.pickle', 'rb') as f:
#     model= pickle.load(f)
