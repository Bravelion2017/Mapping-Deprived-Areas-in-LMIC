...
#%%
# Helper function
from toolbox import get_train_val_ps, important_features, cm, calculate_vif, contextual_features
imp_f= important_features()
contextual_features= contextual_features()
#%%
# Load Data into Pandas Dataframe
lagos= pd.read_parquet('lagos_ctx100m_df.parquet.gzip')
...
pcs['features']= contextual_features
...
most_important_features= important.features.unique() #96
important_96= important.features.unique()
important_72= important_96[:72]
important_100= pcs.features[:100].values
...
plt.axvline(x = 90, color = 'b', label = 'axvline - full height')
...
print(f'With 80 feature size, you get {round(pca_df.head(80).iloc[-1,-1],4)*100}% variance explained')
print(f'With 96  feature size, you get {round(pca_df.head(96).iloc[-1,-1],4)*100}% variance explained')
print(f'With 100  feature size, you get {round(pca_df.head(100).iloc[-1,-1],4)*100}% variance explained')
#%%
# Trying SVD again with most important 96 features
X1= X.loc[:,X.columns.isin(important_96)]
# computing singular values using numpy
...
#%%
plt.figure(figsize=(25,10))
sns.barplot(data=pcs,x=list(range(1,145)),y='PC1')
...
print(f'With 22  feature size, you get {round(pca_df.head(22).iloc[-1,-1],4)*100}% variance explained')

#%%
# LOGISTIC REGRESSION & RANDOM FOREST (96 FEATURES)
train_c= train.loc[:,train.columns.isin(np.append(important_96,'labels'))]
val_c= validation.loc[:,validation.columns.isin(np.append(important_96,'labels'))]
test_c= test.loc[:,test.columns.isin(np.append(important_96,'labels'))]
...
#%%
# LOGISTIC REGRESSION & RANDOM FOREST (72 FEATURES)
train_c= train.loc[:,train.columns.isin(np.append(important_72,'labels'))]
val_c= validation.loc[:,validation.columns.isin(np.append(important_72,'labels'))]
test_c= test.loc[:,test.columns.isin(np.append(important_72,'labels'))]
# Split into features and target
x_train, y_train= train_c.iloc[:,:-1], train_c.iloc[:,-1].values
x_val, y_val= val_c.iloc[:,:-1], val_c.iloc[:,-1].values
x_test, y_test= test_c.iloc[:,:-1], test_c.iloc[:,-1].values
