...
nairobi= pd.read_parquet('nairobi_covariate_df.parquet.gzip')
print(nairobi.describe())
...
most_important_features= important.features.unique() #38
...
print(f'With 38  feature size, you get {round(pca_df.head(38).iloc[-1,-1],4)*100}% variance explained')
...
k_features_test= k_features_test.loc[:,k_features_test.columns.isin(most_important_features)]
k_features= k_features.loc[:,k_features.columns.isin(most_important_features)]
...
#%%
# LOGISTIC REGRESSION & RANDOM FOREST & XGBOOST (38 FEATURES)
...
#%%
# LOGISTIC REGRESSION & RANDOM FOREST & XGBOOST (53 FEATURES)
# Split into features and target
...
#%%
# LOGISTIC REGRESSION & RANDOM FOREST & XGBOOST (10 FEATURES (most important from VIF))
...
for acronym in pipes.keys():
    ...
    # Write cv_results file
    cv_results.to_csv(acronym + '10.csv',index=False)
